...
parser.add_argument('-an', '--anchor_num', default=70, type=int, metavar='N', help='Number of anchors (default: 100)')
...
def rampdown(epoch, num_epochs=100, rampdown_length=30):
...
def pre_train(dataloader, test_loader, dict_loader, dataloader_test, mask_labels, total_epochs = 100, use_gpu=True, seed=123):
    ...
    for epoch in range(start_epoch, total_epochs):    
        ...
        learning_rate =  rampup_value*rampdown_value*0.00003
        adam_beta1 = step_rampup(epoch) * 0.9 + (1.0 - step_rampup(epoch)) * 0.5
        ...
        for iteration, data in enumerate(dataloader, 0):
            ...
            if index.shape[0] == args.batch_size:
                ...
                y = F.sigmoid(48/args.num_bits*0.1*torch.matmul(y_h1, y_h2.permute(1,0)))
...
def main():
    ...
    trainData, trainLabel, testData, testLabel  = du.split_data(image_datasets, select_num=9000)
    ...
    labeled_trainData_75, labeled_trainLabel_75, labeled_trainData_25, labeled_trainLabel_25 = du.split_class(trainData[labeled_idx,:,:,:], trainLabel[labeled_idx], selected_class=idx)

    testData_75, testLabel_75, testData_25, testLabel_25 = du.split_class(testData,testLabel, selected_class=idx)

    anchor_idx = du.select_anchors_1(labeled_trainLabel_75, anchor_num=args.anchor_num)


    print("labeled_idx is:{}".format(labeled_idx.size))
    print("anchor_idx is:{}".format(anchor_idx.size))
    
    print("unique of labeled_trainLabel_25 is:{}".format(np.unique(labeled_trainLabel_25)))

    print("unique of testLabel_25 is:{}".format(np.unique(testLabel_25)))

    dict_data = DT(trainData=labeled_trainData_75[anchor_idx,:,:,:], trainLabel=labeled_trainLabel_75[anchor_idx], transform=transformations_train)
    dict_loader = torch.utils.data.DataLoader(dict_data, batch_size=args.anchor_num, shuffle=False, num_workers=args.workers)


    c_trainData = np.concatenate((trainData[unlabeled_idx,:,:,:], labeled_trainData_75), axis=0)
    c_trainLabel = np.concatenate((trainLabel[unlabeled_idx], labeled_trainLabel_75), axis=0)
    ...
    train_data = DT(trainData=c_trainData, trainLabel=c_trainLabel, transform=transformations_train)
    test_data = DT(trainData=testData_25, trainLabel=testLabel_25, transform=transformations_test)

    labeled_trainData_25 = np.concatenate((labeled_trainData_25,testData_75), axis=0)
    labeled_trainLabel_25 = np.concatenate((labeled_trainLabel_25,testLabel_75), axis=0)

    train_data_test = DT(trainData=labeled_trainData_25, trainLabel=labeled_trainLabel_25, transform=transformations_test)
    train_loader_test = torch.utils.data.DataLoader(train_data_test, batch_size=100, shuffle=True, num_workers=args.workers)

    batch_sampler = data.TwoStreamBatchSampler(
            unlabeled_idx, labeled_idx, args.batch_size, 50)
