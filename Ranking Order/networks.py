...
class Custom_Loss(torch.nn.Module):
    ...
    def forward(self, out, out_h1, out_h2, target, W, batch_target, batch_W, z_h1,z_h2, mask_flag, w_mse, epoch):
        ...
        def e_cross_entropy_mse(out_h1, out_h2, z_h1, z_h2, epoch):
            if epoch>0:
                bn_1 = out_h1.size(1)//2
                bn_2 = out_h2.size(1)//2
                
                Y_prob = F.sigmoid(torch.matmul(out_h1, out_h2.permute(1,0)))
                S_prob = F.sigmoid(torch.matmul(z_h1, z_h2.permute(1,0)))
        ...
        m_loss = e_cross_entropy_mse(out_h1, out_h2, z_h1, z_h2, epoch)

        return sup_loss + w_mse*m_loss
