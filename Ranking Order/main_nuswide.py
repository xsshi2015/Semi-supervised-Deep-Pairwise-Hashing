...
parser.add_argument('-ds', '--dataset', metavar='FILE', help='dataset to use [cifar100, nuswide, coco, cocosent] (default: cifar10)', default='nuswide')
...
parser.add_argument('-nb', '--num_bits', default='24,32,48', type=str, metavar='N', help='Number of binary bits to train (default: 8)')
parser.add_argument('-nc', '--num_class', default=21, type=int, metavar='N', help='Number of classes to train (default: 10)')
parser.add_argument('-an', '--anchor_num', default=126, type=int, metavar='N', help='Number of anchors (default: 100)')
...
def save_checkpoint(state, is_best, prefix='', num_bits=8, filename='./checkpoint/checkpoint.pt'):
    ...
    if is_best:
        ...
        best_filename = os.path.join(path, prefix+'model_best_'+'nuswide_'+str(num_bits)+'.pt')
...
def pre_train(dataloader, test_loader, dict_loader, dataloader_test, dict_loader_test, mask_labels, num_bits, total_epochs = 100, use_gpu=True, seed=123):
    ...
    Z_h2 = torch.zeros(anchor_num, num_bits).float().cuda()        # intermediate values
    z_h2 = torch.zeros(anchor_num, num_bits).float().cuda()        # temporal outputs
    h2 = torch.zeros(anchor_num, num_bits).float().cuda()  # current outputs
    ...
    for epoch in range(start_epoch, total_epochs):    
        ...
        learning_rate =  rampup_value*rampdown_value*0.00003          # In many cases, 0.00005 might obtain better accuracy
        ...
        for iteration, data in enumerate(dataloader, 0):
            ...
            index = torch.squeeze(index).long().cuda()
            ...
            if index.shape[0] == args.batch_size:
                ...
                zcomp_h1 = Variable(zcomp_h1, requires_grad=False)
                zcomp_h2 = Variable(zcomp_h2, requires_grad=False)
                ...
                y = F.sigmoid(torch.matmul(y_h1, y_h2.permute(1,0)))


                loss = criterion(y, y_h1, y_h2, anchor_batch_S, anchor_batch_W, labeled_batch_S, labeled_batch_W, zcomp_h1, zcomp_h2, mask_flag, u_w, epoch)
                ...
                Z_h2[anchor_index,:] = alpha_1 * Z_h2[anchor_index,:] + (1. - alpha_1) * h2[anchor_index,:]
                z_h2[anchor_index,:] = Z_h2[anchor_index,:] * (1. / (1. - alpha_1 ** (epoch + 1)))
        ...
        if epoch % 1 ==0:
            ACG, NDCG = helpers.validate(model, dict_loader_test, dataloader_test, test_loader)


            print("Test image ndcg is:{}".format(NDCG))

            is_best = NDCG > best_prec
            best_prec = max(best_prec, NDCG)
...
def main():
    ...
    transformations_train = transforms.Compose([
        ...
        transforms.ToTensor(),
        transforms.Normalize([0.437, 0.422, 0.383], [0.279, 0.268, 0.288])
        ])
    ...
    retrieval_idx, test_idx = du.split_multi_idx(Label, select_num=2100)
    ...
    unlabeled_idx, labeled_idx = du.split_multi_idx(trainLabel, select_num=10500)
    ...
    # mask_labels = np.squeeze(np.ones((labeled_idx.size, 1)))           # supvised only
    ...
    dict_data_transform_test = folder.dataBags(Path=dict_path, Label=trainLabel[dict_idx,:], transform=transformations_test)
    ...
    #train_data_transform=  folder.dataBags(Path= labeled_train_path, Label=trainLabel[labeled_idx,:],transform=transformations_train)              # supervised only
    train_data_transform_test =  folder.dataBags(Path= labeled_train_path, Label=trainLabel[labeled_idx,:],transform=transformations_test)
    ...
    dict_loader_test = torch.utils.data.DataLoader(dict_data_transform_test, batch_size=args.anchor_num,
                                            shuffle=False, num_workers=args.workers)
    ...
    ####################################################################################
    # Supervised only
    # train_loader = torch.utils.data.DataLoader(train_data_transform, batch_size=100,
    ...
    bits = [int(bit) for bit in args.num_bits.split(',')]
    for bit in bits:
        model = pre_train(train_loader, test_loader, dict_loader, train_loader_test, dict_loader_test, mask_labels, bit, total_epochs = 50, use_gpu=True, seed=args.seed)
