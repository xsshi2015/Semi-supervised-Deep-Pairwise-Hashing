...
#         def l_cross_entropy_pair(Y_h, S,W, mask_flag, epoch, num_bits):
#             # if epoch>0:
#             cond = (mask_flag > 0)
#             nnz = torch.nonzero(cond)
#             nbsup = len(nnz)
#             if nbsup > 0:
#                 bn= nbsup
#                 Y_prob = F.sigmoid(48/num_bits*0.4*torch.matmul(Y_h[cond,:], Y_h[cond,:].permute(1,0)))
...
#                 loss0 = neg_log_likelihood.sum()/neg_log_likelihood.data.nelement()
...
#         def e_cross_entropy_mse(out_h1, out_h2, z_h1, z_h2, epoch, num_bits):
#             if epoch>0:
#                 bn_1 = out_h1.size(1)//2
#                 bn_2 = out_h2.size(1)//2
                
#                 Y_prob = F.sigmoid(48/num_bits*0.4*torch.matmul(out_h1, out_h2.permute(1,0)))
#                 S_prob = F.sigmoid(48/num_bits*0.4*torch.matmul(z_h1, z_h2.permute(1,0)))
    ...
    def forward(self, out, out_h1, out_h2, target, W, batch_target, batch_W, z_h1,z_h2, mask_flag, w_mse, epoch, bit):
        def cross_entropy_objective(Y_prob, Y, W, mask_flag):
            ...
            if nbsup > 0:
                ...
                pn = torch.sum(Y).float()
                nn = -1.0*torch.sum(W*(1. - Y)).float()

                
                neg_log_likelihood = -1. * (nn/pn)*Y * torch.log(s_Y_prob) + W*(1. - Y) * torch.log(1. - s_Y_prob)  # negative log bernoulli 


                loss = neg_log_likelihood.sum()/(2*nn)
        ...
        def l_cross_entropy_pair(Y_h, S,W, mask_flag, epoch, num_bits):
            ...
            if nbsup > 0:
                ...
                pn = torch.sum(S1)
                nn = -1.0*torch.sum(W1*(1. - S1))
                
                neg_log_likelihood = -1. *(nn/pn)*S1* torch.log(s_Y_prob) + W1*(1. - S1) * torch.log(1. - s_Y_prob)  # negative log bernoulli 

                loss0 = neg_log_likelihood.sum()/(2*nn)
