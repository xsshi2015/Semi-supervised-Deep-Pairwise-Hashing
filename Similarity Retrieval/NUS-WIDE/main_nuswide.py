...
parser.add_argument('-nb', '--num_bits', default=48, type=int, metavar='N', help='Number of binary bits to train (default: 48)')
parser.add_argument('-nc', '--num_class', default=21, type=int, metavar='N', help='Number of classes to train (default: 10)')
parser.add_argument('-an', '--anchor_num', default=420, type=int, metavar='N', help='Number of anchors (default: 100)')
...
def EncodingOnehot(target, nclasses=10):
    if target.size(0)>1:
        ...
        target_onehot = torch.FloatTensor(torch.zeros(1, nclasses))
...
def pre_train(dataloader, test_loader, dict_loader, dataloader_test, mask_labels, total_epochs = 50, learning_rate=1e-4, use_gpu=True, seed=123):
    ...
    for epoch in range(start_epoch, total_epochs):    
        ...
        for iteration, data in enumerate(dataloader, 0):
            ...
            if index.shape[0] == args.batch_size:
                ...
                y = F.sigmoid(48/num_bits*0.4*torch.matmul(y_h1, y_h2.permute(1,0)))
...
def main():
    ...
    model = pre_train(train_loader, test_loader, dict_loader, train_loader_test, mask_labels, total_epochs = 50, learning_rate=1e-4, use_gpu=True, seed=args.seed)
